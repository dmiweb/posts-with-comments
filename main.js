...
;// ./src/js/components/util/converterTimestamp.js
...
/* harmony default export */ const util_converterTimestamp = (converterTimestamp);
;// ./src/js/components/post/Post.js
...
class Post {
  ...
  renderPost() {
    this.stream$.subscribe(response => {
      ...
      const date = util_converterTimestamp(created);
...
;// ./src/js/components/comment/Comment.js
...
function fromAjax(init) {
    return new Observable_Observable(function (destination) {
        var _a, _b;
...
;// ./src/js/components/api/Stream.js


class Stream {
  create() {
    const stream$ = interval().pipe(take(5), concatMap(() => {
      return ajax("https://posts-with-comments-backend.vercel.app/posts/latest").pipe(map(({
        response
      }) => {
        ...
        const post = posts[0];
        return ajax(`https://posts-with-comments-backend.vercel.app/posts/${post.id}/comments/latest`).pipe(map(({
          response
        }) => {
          return {
            post,
